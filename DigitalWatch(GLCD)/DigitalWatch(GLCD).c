...
// HW1: 디지털 시계 제작
// 제출자: 2015132012 박근우
// 개요: TIMER를 이용한 스탑 워치 기능이 있는 디지털 시계 제작 
//         TIM3는 UpCounting, TIM4는 ch3에서 OC모드로 설정  스탑워치는 중단 상태일때만 초기화가 가능
#include "stm32f4xx.h"
#include "GLCD.h"
...
int main(void)
{
    ...
    TIMER4_OC_Init();		// 범용타이머(TIM3) 초기화 : up counting mode
    while(1)
    {
...
{
// PB8: TIM4_CH3
// PD12을 출력설정하고 Alternate function(TIM4_CH3)으로 사용 선언
...
// Timerbase 설정
	...
	TIM4->CCMR2 &= ~(1<<3); // OC3PE=0: Output Compare 3 preload disable(CCR1에 언제든지 새로운 값을 loading 가능) 
	...
	TIM4->CCER |= (1<<8);	// CC3E=1: CC3 channel Output Enable
				// OC3(TIM4_CH3) Active: 해당핀(167번)을 통해 신호출력 pb8
	TIM4->CCER &= ~(1<<9);	// CC1P=0: CC3 channel Output Polarity (OCPolarity_High : OC3으로 반전없이 출력)  
...
void _EXTI_Init(void) //EXTI 12,13으로 수정
{
...
void EXTI15_10_IRQHandler(void)		// EXTI 15~10 인터럽트 핸들러
{
	if(EXTI->PR & 0x1000) 		// EXTI12 nterrupt Pending?
	{
		...
		EXTI->PR |= 0x1000; 	// Pending bit Clear
               GPIOG->ODR ^= 0x10;
               GPIOG->ODR |= 0x20;
               if(GPIOG->ODR == 0x30)           //STOP WATCH 동작
		SW4_Flag = 1;
               else                                     //STOP WATCH  중지
                 SW4_Flag = 0;
...
void DisplayInitScreen(void)
{
    ...
    LCD_DrawRectangle(4, 6, 128, 38); // 사각형 그리기  
